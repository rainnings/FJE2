...
import visitor.RectangleVisitor;
...
public class RectangleStyle extends JsonStyle {
     ...
     * @param root 根节点
     */
    public void getDrawInfo(JsonNode root){
        isFirst = true;
        isLastLeaf = false;
     ...
     * @param isLast
     */
    private void getInfo(JsonNode node, String prefix, boolean isLast){
        node.info.prefix = prefix;
        node.info.isLast = isLast;
        node.info.isFirst = isFirst;
        node.info.isLastLeaf = isLastLeaf;
        ...
        for(int i = 0; i < children.size(); i++){
            ...
            getInfo(children.get(i), newPrefix, isLast && isLastChild);
        }
    }
     ...
     */
    @Override
    public void showIterator(JsonNode root){
        getDrawInfo(root);
        Iterator iterator = root.getDFIterator();
        Visitor visitor = new RectangleVisitor(intermediateIcon, leafIcon, rectangleLength);
        iterator.next();
